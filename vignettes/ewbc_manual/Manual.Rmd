...
In this vignette, you will get more information about the construction of the main-function `pre_process_ew_input()` and `post_process_ewbc_output`. 
The task of `pre_process_ew_input()` is to pre-process the input-parameter-files of ECOWEIGHT for beef cattle (EWBC).
Whereas the task of `post_process_ewbc_output` is to extract from the result file of ECOWEIGHT specific for beef cattle economic weights and population means for a specific scenario.
...
As soon as we have the result file of ECOWEIGHT for beef cattle, we can run the post-processing part to extract economic weights and population means.

# Post-processing steps

The major step of `post_process_ewbc_output` is to read output file from ECOWEIGHT and makes single table with piechart per scenario. 
...
To show how `post_process_ewbc_output` is build, we made some test-file from ECOWEIGHT (EWBC).
