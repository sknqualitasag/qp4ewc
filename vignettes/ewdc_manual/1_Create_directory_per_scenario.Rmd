...
By running the function `create_directory_scenario()` with these parameters is a directory build up such as each scenario corresponds to a directory. In this directory are templates of EWDC (so based on dairy cattle) copied. With the function `update_input_parameter_file()` of this package can the template with values in this created directory updated.
